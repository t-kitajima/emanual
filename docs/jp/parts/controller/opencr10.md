...
- 通信ポート: シリアルポート名は、Linuxでは通常`/dev/ttyACM0`であり、OpenCRに接続されているシリアルポートと同じである必要があります。  
- ボーレート : 通信・入力する速度は115,200bpsです。  
- ファームウェアバイナリ : ファームウェアバイナリイメージがbinの拡張子を持っています。  
- ファームウェア実行状況 : 1 の場合、ファームウェアをダウンロードした後に実行されます。入力されていない場合や、0の場合はファームウェアのダウンロードのみを実行します。  

  ![](/assets/images/parts/controller/opencr10/arduino_bin_export.png)

  > Arduino IDEからコンパイルしたバイナリファイルをエクスポートする

### [Linux/Macの例](#linuxmac-example)

OpenCRがttyACM0ポートに接続されていて、バイナリファイル *opencrfw.bin* がopencr_ldディレクトリにコピーされている場合。  

```bash
$ sudo opencr_ld /dev/ttyACM0 115200 ./opencrfw.bin 1
```

### [Windowsの例](#windows-example)

OpenCRがCOM1ポートに接続されていて、バイナリファイル  *opencrfw.bin* がopencr_ldディレクトリにコピーされている場合。  
